/*
// Hide url and navigation when scrolling
const main = document.getElementsByTagName("main");
document.documentElement.requestFullscreen(); // Set the default to fullscreen
...
main.addEventListener("scroll", () => {
   let newY = main.pageYOffset || document.documentElement.scrollTop;
   if (newY > lastScrollTop) {
      isScrollingDown = true;
      toggleFullScreen();
   } else if (newY < lastScrollTop) {
      isScrollingDown = false;
      toggleFullScreen();
   }
   lastScrollTop = newY <= 0 ? 0 : newY; // For Mobile or negative scrolling
}, false);
*/
